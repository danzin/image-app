...
const ImageView = () => {
	...
	const { data: image, isLoading, isError, error } = usePostById(id || "");
	...
	const toggleFavoriteMutation = useMutation({
		...
		onMutate: async (shouldFavorite) => {
			if (!image) return;
			await queryClient.cancelQueries({ queryKey: ["post", image.publicId] });
			setIsFavorited(shouldFavorite);

			const previousImage = queryClient.getQueryData<IImage>(["post", image.publicId]);

			queryClient.setQueryData<IImage>(["post", image.publicId], (old) =>
		...
		onError: (_err, _shouldFavorite, context) => {
			...
			if (context?.previousImage) {
				queryClient.setQueryData(["post", image.publicId], context.previousImage);
		...
		onSettled: () => {
			if (!image) return;
			queryClient.invalidateQueries({ queryKey: ["post", image.publicId] });
		},
	});
