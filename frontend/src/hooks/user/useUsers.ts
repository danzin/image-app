...
export const useUserImages = (userId: string, options?: UseUserImagesOptions) => {
	return useInfiniteQuery({
		queryKey: ["userImages", userId] as const,
		queryFn: ({ pageParam = 1 }) => fetchUserImages(pageParam as number, userId),
		initialPageParam: 1,
...
export const useEditUser = () => {
	...
	return useMutation({
		...
		onSuccess: (data) => {
			...
			queryClient.setQueryData(["user"], (oldData: object) => ({
				...oldData,
