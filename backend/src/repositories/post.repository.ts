...
export class PostRepository extends BaseRepository<IPost> {
  ...
  async getFeedForUserCoreWithCursor(
    ...
    try {
      ...
      const decodedCursor = decodeCursor<{ phase: "personalized" | "backfill"; createdAt?: string; _id?: string }>(options.cursor);
      let phase: "personalized" | "backfill" = decodedCursor?.phase ?? "personalized";
      ...
      if (hasMore && results.length > 0) {
        ...
        const outgoingPhase = lastItem.isPersonalized ? "personalized" : "backfill";
        nextCursor = encodeCursor({ phase: outgoingPhase, createdAt: lastItem.createdAt, _id: lastItem._id });
      }
