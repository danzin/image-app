...
import { ImageRepository } from './image.repository.old';
...
export class UserRepository implements BaseRepository<IUser> {
  ...
  async followUser(followerId: string, followeeId: string, options?: { session: mongoose.ClientSession}): Promise<void> {  

    // Create the follow relationship
    await this.followRepository.addFollow(followerId, followeeId, options?.session);
  ...
  // Unfollow a user
  async unfollowUser(followerId: string, followeeId: string, options?: { session: mongoose.ClientSession}): Promise<void> {

    // Delete the follow relationship
    await this.followRepository.removeFollow(followerId, followeeId, options?.session);
