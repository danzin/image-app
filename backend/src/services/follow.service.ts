...
export class FollowService {
  ...
  async followUser(
    ...
    if (await this.followRepository.isFollowing(followerId, followeeId))
    ...
    try {
      await this.unitOfWork.executeInTransaction(async () => {
        ...
        await this.userRepository.update(followeeId, {
  ...
  async unfollowUser(
    ...
    if (!(await this.followRepository.isFollowing(followerId, followeeId)))
      return "Already unfollowed this user";
  ...
  private async invalidateFollowerFeedCache(userId: string): Promise<void> {
    try {
      ...
      logger.info(
        `Invalidated feed cache for user ${userId} after follow/unfollow action`,
      );
    } catch (error) {
