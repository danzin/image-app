...
> {
  ...
  async execute(command: DeleteCommentCommand): Promise<void> {
    ...
    try {
      ...
      const effectivePost =
        (await this.postReadRepository.findByIdWithPopulates(
      ...
      const isCommentOwner =
        comment.userId && comment.userId.toString() === user.id;
      const { ownerInternalId: postOwnerInternalId } =
        this.extractPostOwnerInfo(effectivePost);
      ...
      postTags = Array.isArray(effectivePost.tags)
        ? (
            effectivePost.tags as (mongoose.Types.ObjectId | PopulatedPostTag)[]
          ).map((t) =>
        ...
        : [];
      const { ownerPublicId: postOwnerPublicId } =
        this.extractPostOwnerInfo(effectivePost);
      if (!postOwnerPublicId && postOwnerInternalId) {
        const ownerDoc =
          await this.userRepository.findById(postOwnerInternalId);
