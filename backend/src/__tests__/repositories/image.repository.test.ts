...
describe("ImageRepository", () => {
	...
	describe("findWithPagination", () => {
		...
		it("should throw DatabaseError on find query failure", async () => {
			...
			await expect(repository.findWithPagination(options)).to.be.rejectedWith("DatabaseError");
		...
		it("should throw DatabaseError on count query failure", async () => {
			...
			await expect(repository.findWithPagination(options)).to.be.rejectedWith("Count failed");
		});
	});
	...
	describe("findByUserId", () => {
		it("should return images for a user with default pagination", async () => {
			...
			expect(mockFindQuery.sort.calledOnceWith({ createdAt: "desc" })).to.be.true;
		...
		it("should throw DatabaseError on findByUserId failure", async () => {
			...
			await expect(repository.findByUserId(mockUserId, options)).to.be.rejectedWith(dbError.message);
		});
	});
});
